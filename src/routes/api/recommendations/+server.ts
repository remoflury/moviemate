...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const recommendations: TMDBMovieByRecommendationProps[] = await getMovieRecommendationsById(
		'800089',
		TMDB_BASE_URL,
		TMDB_AUTH_KEY,
		1
	...
	let filteredRecommendations = recommendations.filter(
		(movie) => !watchlistMovieIds.includes(movie.id.toString())
	);
	// filter out movies, which are in dismissed list
	filteredRecommendations = filteredRecommendations.filter(
		(movie) => !dismissedMovieIds.includes(movie.id.toString())
	);
