export const getMatchesAndNotMatchesArray = <T>(array: T[]) => {
  const counts = array.flatMap(item => item.movies_watchlist).reduce((acc, value) => {
...
export const getMovieRecommendationsById = async (movieId: string, tmdbUrl: string, tmdbAutKey: string, page: number = 1) => {
  const response = await fetch(`${tmdbUrl}/movie/${movieId}/recommendations?page=${page}&language=en-US`, {
    method: 'GET',
    headers: {
      accept: 'application/json',
      Authorization: `Bearer ${tmdbAutKey}`
